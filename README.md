...
2. Rolling `Duality` with special roll types like `Advantage`, `Disadvantage`, or `Best of 3` is currently unsupported. I recommend against doing this. `Critical` should mostly handle it and only do modifications to the results of non-Duality groups, like **2x Total** should only apply to non-Duality groups.
